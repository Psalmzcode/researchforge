  ...
  async function deliver() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFiles([])
        router.refresh()
        toast.success('Deliverables uploaded. Admin will review before the client is notified.')
      } else {
      ...
      {done ? (
        ...
        <>
            ...
            <div className="space-y-1">
              {files.map((f, i) => (
                <div key={i} className="flex items-center justify-between text-xs p-2 rounded-lg" style={{ background: 'rgba(0,198,162,.05)' }}>
                  <span>📄 {f.name}</span>
                  <button type="button" onClick={() => setFiles(f => f.filter((_, j) => j !== i))} className="text-red-400 ml-2">
                    ✕
                  </button>
                </div>
              ))}
          ...
          <button
            ...
            className="inline-flex w-full items-center justify-center gap-2 py-2.5 rounded-full text-xs font-bold disabled:opacity-50 transition-all hover:-translate-y-0.5"
            style={{ background: 'var(--accent)', color: 'var(--text-on-accent)' }}
          >
            {loading ? (
              <>
                <Spinner size="sm" label="Uploading deliverables" />
                <span>Uploading & Sending…</span>
              </>
            ) : (
              `📦 Deliver ${files.length > 0 ? `${files.length} file(s)` : 'Work'}`
            )}
          </button>
          <p className="text-[10px] text-center" style={{ color: 'var(--muted)' }}>
            Files go to admin for review first, then client is notified
          </p>
        </>
      )}
    </div>
