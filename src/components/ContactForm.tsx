...
export function ContactForm() {
  ...
  const inputStyle =
    'w-full rounded-xl px-4 py-3 text-[.9rem] outline-none transition-all duration-200 border focus:border-[var(--accent)] focus:bg-[rgba(0,198,162,.05)]'
  const inputBase = { background: 'rgba(255,255,255,.05)', borderColor: 'var(--card-border)', color: 'var(--text)' }

  return (
    <form
      onSubmit={handleSubmit}
      className="rounded-[28px] p-10 text-left border"
      style={{ background: 'rgba(255,255,255,.04)', borderColor: 'var(--card-border)' }}
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <div className="flex flex-col gap-1.5 sm:col-span-2">
          ...
          <textarea
            required
            rows={4}
            placeholder="Describe your challenge, goal, or what you need help with..."
            style={inputBase}
      ...
      <button
        ...
        className="inline-flex w-full items-center justify-center gap-2 py-4 rounded-full text-[.93rem] font-bold border-none cursor-pointer transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(0,198,162,.3)] disabled:opacity-70 disabled:cursor-not-allowed"
        style={{
          background: status === 'sent' ? '#00a882' : 'var(--accent)',
